...
В `desktop/app/package.json` и `package-lock.json` указана версия `0.1.16`. Локальная Windows-сборка выполняет typecheck, тесты, production build, проверку packaging-контракта, NSIS-сборку и проверку готовых артефактов:
...
Для `0.1.16` ожидаются:

- `release/Counterpick-0.1.16-x64.exe`
- `release/Counterpick-0.1.16-x64.exe.blockmap`
- `release/latest.yml`
