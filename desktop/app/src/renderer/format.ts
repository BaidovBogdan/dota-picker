...
export const phaseCopy = (phase: EnginePhase, language: Language = 'en') => {
  const copy: Record<Language, Record<EnginePhase, { title: string; description: string }>> = {
    ru: {
      ...
      recognizing: { title: 'Распознаём героев', description: 'Анализируем только окно Dota 2 и сверяем найденные портреты.' },
    ...
    en: {
      ...
      recognizing: { title: 'Recognizing heroes', description: 'Analyzing only the Dota 2 window and matching hero portraits.' },
