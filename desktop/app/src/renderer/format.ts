...
export const phaseCopy = (phase: EnginePhase, language: Language = 'en') => {
  const copy: Record<Language, Record<EnginePhase, { title: string; description: string }>> = {
    ru: {
      ...
      recognizing: { title: 'Распознаём героев', description: 'Сверяем портреты в кадре Dota 2; локальный GSI помогает определить фазу, команду и сторону по выбранному герою.' },
    ...
    en: {
      ...
      recognizing: { title: 'Recognizing heroes', description: 'Matching portraits in the Dota 2 frame while local GSI identifies phase, team, and side from your selected hero.' },
